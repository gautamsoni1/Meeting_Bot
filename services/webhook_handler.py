...
# #     #     print("   (unhandled event — ignoring)")
# #     #     return
...
# #     # call_ended = meeting is over, recording should be processing
...
# #     """
# #     Poll bot until audio_mixed_mp3.status == done,
# #     then download and store the MP3.
# #     """
...
# #             # ✅ Now correctly reads from bot.recordings[].media_shortcuts
...
# #         # Build public ngrok URL
# #         filename   = f"{bot_id}.mp3"
# #         # public_url = f"{NGROK_URL}/download/{filename}"
# #         if not NGROK_URL:
# #            raise ValueError("❌ NGROK_URL is empty — cannot build public URL")

# #         public_url = f"{NGROK_URL}/download/{filename}"

# #         store.upsert(bot_id, status="done", local_file=local_file, public_url=public_url)
...
# #         print(f"   ❌ Error processing recording for bot {bot_id}: {exc}")
# #         store.upsert(bot_id, status="error")
...
#     # ✅ Mark as fully done — audio only, no report generation
#     store.upsert(
#         bot_id,
#         status="done",
#         report_status="done",
...
#     print(f"   Local  : {local_file}")
#     print(f"   Public : {public_url}\n")
#     # ✅ Stopped here — no transcription, no PDF report
